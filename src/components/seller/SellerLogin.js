...
export default function Login({why,setwhy,notification}) {
  ...
  let handleclick = async (e)=>{
    ...
    let response = await fetch("http://127.0.0.1:5000/admin/Login", {
      method: "POST", 
      headers: {
  ...
  if(response.status===200){
     ...
     setwhy(why=>why+1)
     notification("blue",`Successfully logged in as ${data.name}`)
     history('/home')
  }
  else {
    alert(data.message)
